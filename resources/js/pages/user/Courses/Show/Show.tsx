...
interface CoursesDetailPageProps {
    ...
    reviews: Review[];
    purchases: {
        data: Purchase[];  // Added data structure
    };
...
export default function CourseDetailPage({ course, purchases, totalPurchasesCount, userProgressPercentage }: CoursesDetailPageProps) {

    const { course: data, applied_coupon } = usePage<PageProps>().props;

    // find quiz info

    const totalQuizzess = course?.modules?.map((module: Module) => module?.quiz).length
    ...
    const discountAmount = applied_coupon?.discount_type === 'percentage'
        ? (course.price * applied_coupon?.discount_value) / 100
        : applied_coupon?.discount_value || 0;

    const finalPrice = (course.price - discountAmount).toFixed(2);
    ...
    const averageRating = course?.reviews?.length > 0
        ? (course.reviews?.reduce((sum, review: Review) => sum + review.rating, 0) / course?.reviews?.length)
        : 0;
    ...
    // Course stats
    const totalLessons = course.modules.reduce((acc: any, module: Module) => acc + module.lessons.length, 0);
    const totalDuration = course?.modules?.reduce((acc: any, module: Module) => acc + module.duration, 0) || 0;

    const formatReviewCount = (count: any) => {
        if (!count) return '0';
