...
export function QuizContainer({ quiz, courseId, onQuizComplete, initialAttempts = 0 }: QuizProps) {
    ...
    const handleQuizSubmit = async () => {
        // First calculate and show results
        const score = calculateQuizScore();
        setQuizScore(score);
        setQuizSubmitted(true);
        setTimerActive(false);
        ...
        // Then automatically upload progress
        setUploading(true);
        ...
        try {
            await router.post('/user/course-progress/store-or-update', {
                quiz_id: quiz.id,
                course_id: Number(courseId),
                total_questions: quiz.questions.length,
                answered_questions: Object.keys(quizAnswers).length,
                correct_answers: quiz.questions.filter(
                    q => quizAnswers[q.id] === q.correct_answer
                ).length,
                score: score,
                max_score: totalPossiblePoints,
                attempt_number: quizAttempts,
                passed:  score >= quiz.passing_score ? true : false,
                completed: true,
            }, {
                onSuccess: () => {
                    setUploaded(true);
                    ...
                    // // Optional: Auto-navigate if passed
                    // if (isQuizPassed) {
    ...
    return (
        <div className="space-y-6">
            ...
            {quiz.questions.map(question => (
                <Card
                    key={question.id}
                    className={`transition-colors ${quizSubmitted &&
                    ...
                    <CardContent>
                        ...
                        >
                            {question.options.choices.map(choice => (
                                <div
                                    key={choice.key}
                                    className={`flex items-center space-x-3 p-3 rounded-lg ${quizSubmitted &&
            ...
            {!quizSubmitted ? (
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className={`text-center text-lg font-medium ${isQuizPassed ? 'text-green-500' : 'text-red-500'
                            }`}>
